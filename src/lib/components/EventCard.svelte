...
<article class="bg-black border border-neutral-800 rounded-md overflow-hidden max-w-full mb-6">
    ...
    <div class="flex items-center justify-between px-3 py-2.5">
        <div class="flex space-x-4">
            <!-- Like -->
            <button onclick={handleLike} class="hover:scale-105 active:scale-95 transition-all text-neutral-100 hover:text-red-500">
            ...
            <!-- Comment -->
            <button onclick={toggleComments} class="hover:scale-105 active:scale-95 transition-all text-neutral-100 hover:text-blue-500">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
            </button>
            <!-- Share -->
            <button class="hover:scale-105 active:scale-95 transition-all text-neutral-100 hover:text-green-500">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <!-- Bookmark -->
        <button onclick={handleBookmark} class="hover:scale-105 active:scale-95 transition-all text-neutral-100 hover:text-yellow-600">
    ...
    <div class="px-3 pb-2 text-xs leading-relaxed">
        ...
        {:else}
            ...
            {#if isTruncated}
                <button onclick={() => isExpanded = true} class="text-neutral-400 hover:text-neutral-300 ml-1 font-medium focus:outline-none">
                    ...more
                </button>
            {/if}
        {/if}
    ...
    <div class="px-3 pb-2">
        <button onclick={toggleComments} class="text-[11px] text-neutral-400 hover:text-neutral-300 transition-colors focus:outline-none">
